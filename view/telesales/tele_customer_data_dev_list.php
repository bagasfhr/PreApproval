...
$ws_ip				= "http://10.1.49.220:7675";

$SesIP              = $ws_ip;

$SesIP              = $ws_ip;
if (substr($SesExt,0,3) == '531') {
	$SesIP = "http://10.1.49.220:7675";
}else if (substr($SesExt,0,3) == '532') {
	$SesIP = "http://10.1.49.221:7675";
...
if ($mode=="es") {
	$iddet = "105037"; //"51162"; //diganti ganti aja!
	$wsql  = " AND a.id='$iddet'";
	$sql_whrlist = "";
}
...
if($iddet!=''){
	...
	$sqldis = "SELECT COUNT(a.id) AS total_main,sum(if(b.call_status>0,1,0)) AS total_temp FROM cc_ts_penawaran a 
			   ...
			   ON a.id=b.id
			   WHERE (a.customer_id='$txt_customer_no' OR a.customer_id_ro='$txt_customer_no') AND a.is_eligible_crm= '1'
	...
	// model customer
	$sql_cust_model = "SELECT cust_model_name, cust_type FROM cc_master_customer_model WHERE is_active=1";
									...
									<?php
										while($row = mysqli_fetch_array($res_cust_model)){
											if ($row['cust_model_name'] == $txt_job_modelname) {
												echo "<option value='".$row['cust_model_name']."' data-type='".$row['cust_type']."' selected>".$row['cust_model_name']."</option>";
											}else{
												echo "<option value='".$row['cust_model_name']."' data-type='".$row['cust_type']."' >".$row['cust_model_name']."</option>";
											}
										}
										mysqli_data_seek($res_ownership, 0);
									?>
...
if(txtphonenoadd1!='' )
    ...
    $("#btnSaveForm").click(function(){ 
    	var param_dukcapil2= document.getElementById("txt_activity_ducapil").value;
    	 ...
    	 // var url = "http://10.1.49.250/wom/index.php?v=dGVsZXNhbGVzfHRlbGVfY3VzdG9tZXJfZGF0YV9kZXZfbGlzdHxDdXN0b21lciBEYXRhIERldnwzMDZ8";
    	 // var url = "http://10.1.49.250/wom/index.php?v=dGVsZXNhbGVzfHRlbGVfY3VzdG9tZXJfZGF0YV9kZXZfbGlzdHxDdXN0b21lciBEYXRhIERldnwzMDZ8";
    	 var url = "http://10.1.49.250/wom/index.php?v=dGVsZXNhbGVzfHRlbGVfY3VzdG9tZXJfZGF0YV9kZXZfbGlzdHxDdXN0b21lciBEYXRhIERldnwzMDZ8";  
	...
	$("#three_pro_offering").change(function() {
		...
		$("#btn_duckcapil").click(function() {
			...
			if (param_dukcapil==0) {
				...
				var urlcheckcust	   = "service/wom/check_negative.php?nik="+nik+"&customer_id="+cust_no+"&source="+source+"&PreApproval="+IS_PRE_APPROVAL+"&app_no="+app_no;
					$.ajax({
							url: urlcheckcust,
							type: "POST",
							dataType: 'json',
							success: function(data2){ 
								...
								console.log("masuk")
								if (datarnegres == 'Match' || datarnegres == 'MATCH') {
			    ...
			    success: function(d) {
		        	...
		        	for (var i = 0; i < row_bulk.length; i++) {
		            	if (row_bulk[i].checked == true) {
		            		...
		            		// var url = "http://10.1.49.250/wom/index.php?v=dGVsZXNhbGVzfHRlbGVfY3VzdG9tZXJfZGF0YV9kZXZfbGlzdHxDdXN0b21lciBEYXRhIERldnwzMDZ8";  
		            		var url = "http://10.1.49.250/wom/index.php?v=dGVsZXNhbGVzfHRlbGVfY3VzdG9tZXJfZGF0YV9kZXZfbGlzdHxDdXN0b21lciBEYXRhIERldnwzMDZ8"; 
		            		// var url = "http://10.1.49.250/wom/index.php?v=dGVsZXNhbGVzfHRlbGVfY3VzdG9tZXJfZGF0YTJfZGV2X2xpc3R8Q3VzdG9tZXIgRGF0YSBEZXYgU3lzfDMwOXw%3D"; 
							url += '&param='+row_bulk[i].value;
							window.location.href = url; 
		             	}
		             	else{
