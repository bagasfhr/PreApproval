...
if($v==="call_status"){ 
	...
	if($skillid !== "") {
		...
		if($skillid === "0") {
			...
			if ($sqlrec1 = mysqli_fetch_object($sqlres1)) {
	...
	echo json_encode($aReturn);
        ...
        $sql1 = "SELECT a.id, a.campaign_name FROM cc_campaign_prv a WHERE a.category_id = '$campid' AND a.status = 1 ORDER BY a.id ASC "; 
		$res1 = mysqli_query($condb, $sql1);
		$arr_category[] = "<option value=''>-- Select --</select>";
		while($row1 = mysqli_fetch_object($res1)) {
			@extract($row1, EXTR_OVERWRITE);
	...
	echo json_encode($aReturn);
	    ...
	    if($skillid !== "") {
			...
			a.agent_id=b.id  AND c.group_id='$v_agentgroup' $whr_search $whr group by b.agent_name order by b.agent_name ASC";
		    ...
		    while (($sql_rec1 = mysqli_fetch_object($sql_res1)) === TRUE) {
			    ...
			    if ($sql_rec12 = mysqli_fetch_object($sql_res12)) {
		    ...
		    while (($sql_rec2 = mysqli_fetch_object($sql_res2)) === TRUE) {
		        $id       	= $sql_rec2->id;  
...
echo json_encode($aReturn);
	...
	echo json_encode($aReturn);
		...
		if ($sql_rec13 = mysqli_fetch_object($sql_res13)) {
			...
			$sqldel = "DELETE FROM cc_ts_assign WHERE agent_id='$searchid' AND create_by='$v_agentid'";
...
echo json_encode($aReturn);
	...
	$sql_str1 = "SELECT id, suppl_name FROM cc_master_supplier WHERE 1=1 $sql_whr";
	$sql_res1  = execSQL($condb, $sql_str1);
...
echo json_encode($aReturn);
    ...
    if($prov === "" || $prov === "0"){
    	...
    	while($row = mysqli_fetch_object($res_kabupaten)){
		...
		while($row=mysqli_fetch_object($res_legal_kab)){
				$sel .= "<option value='".$row["city"]."'>".$row["city"]."</option>";
		}
